...
- 🤖 **智能对话** - LangChain 多轮对话 + 流式输出（待定）
- 📚 **RAG 问答** - 向量检索增强，支持文档上传、自动建立向量索引、自动更新知识库（待定）
- 🔧 **AIOps 诊断** - Plan-Execute-Replan 自动故障诊断和根因分析（开发中）
- 🌐 **Web 界面** - 现代化 UI，支持多种对话模式：快速问答/流式对话（完成）
- 🔌 **MCP 集成** - 日志查询和监控数据工具接入（自定义）
