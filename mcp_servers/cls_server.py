...
mcp = FastMCP("CLS", host="127.0.0.1", port=8003)
...
def get_mock_data_latest_timestamp() -> int:
    """从模拟数据文件中获取最新的时间戳（毫秒）。

    读取 data/logs.json 文件，找到最新的时间戳，并转换为毫秒。
    ...
    try:
        ...
        logs_file = os.path.join(project_root, "data", "logs.json")

        if not os.path.exists(logs_file):
            logger.warning(f"模拟数据文件不存在: {logs_file}，使用系统时间")
        ...
        if not logs:
            logger.warning("模拟数据为空，使用系统时间")
            return int(datetime.now().timestamp() * 1000)

        # 找到最新的时间戳
        latest_time_str = logs[-1].get("timestamp", "")
        if not latest_time_str:
            logger.warning("无法找到时间戳，使用系统时间")
            return int(datetime.now().timestamp() * 1000)

        # 解析 ISO 8601 格式的时间字符串
        # 例如: "2026-05-19T10:59:53Z"
        dt = datetime.fromisoformat(latest_time_str.replace('Z', '+00:00'))
        timestamp_ms = int(dt.timestamp() * 1000)

        logger.info(f"从模拟数据获取最新时间: {latest_time_str} -> {timestamp_ms}")
        return timestamp_ms

    except Exception as e:
        logger.error(f"读取模拟数据失败: {e}，使用系统时间")
        return int(datetime.now().timestamp() * 1000)
...
def get_current_timestamp() -> int:
    ...
    注意：在模拟数据模式下，返回的是模拟数据中的最新时间戳，而不是系统时间。
    ...
    """
    return get_mock_data_latest_timestamp()
