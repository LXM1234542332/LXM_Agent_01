...
start-cls:
	...
	@if pgrep -f "mcp_servers/cls_server.py" > /dev/null 2>&1; then \
		...
		nohup .venv/bin/python -u mcp_servers/cls_server.py > mcp_cls.log 2>&1 & \
		pid=$$!; \
		echo $$pid > mcp_cls.pid; \
		sleep 3; \
		if pgrep -f "mcp_servers/cls_server.py" > /dev/null 2>&1; then \
...
start-monitor:
	...
	@if pgrep -f "mcp_servers/monitor_server.py" > /dev/null 2>&1; then \
		...
		nohup .venv/bin/python -u mcp_servers/monitor_server.py > mcp_monitor.log 2>&1 & \
		pid=$$!; \
		echo $$pid > mcp_monitor.pid; \
		sleep 3; \
		if pgrep -f "mcp_servers/monitor_server.py" > /dev/null 2>&1; then \
